...
describe('realmReduxSnapshot', () => {
  ...
  // Changes to top-level payloads objects
  describe('changes to top-level payloads', () => {
    ...
    it('should change a Realm.Results value within a plain object payload to an Array', () => {
      let results = testRealm().objects('Dog')
      let initialAction = { type: 'TEST', payload: { results } }
      let result = testMiddleware(initialAction)
      expect(result.payload.results.constructor).to.not.equal(Realm.Results)
      expect(result.payload.results.constructor).to.equal(Array)
    })

    it('should change a Realm.Object value within a plain object payload to a plain object', () => {
      let dog = testRealm().objects('Dog')[0]
      let initialAction = { type: 'TEST', payload: { dog } }
      let result = testMiddleware(initialAction)
      expect(result.payload.dog.constructor).to.not.equal(Realm.Object)
      expect(result.payload.dog.constructor).to.equal(Object)
    })

    it('should not fail if some values within a plain object payload are null or undefined', () => {
      let initialAction = { type: 'TEST', payload: { a: undefined, b: null } }
      let result = testMiddleware(initialAction)
      expect(result.payload.a).to.be.undefined
      expect(result.payload.b).to.be.null
    })

    it('should correctly handle a combination of Realm.Results, Realm.Object and other non-Realm values', () => {
      let dogs = testRealm().objects('Dog')
      let cat = testRealm().objects('Cat')[0]
      let bird = { name: 'A bird' }
      let isAZoo = true
      let initialAction = { type: 'TEST', payload: { dogs, cat, bird, monkey: undefined, isAZoo } }
      let result = testMiddleware(initialAction)
      expect(result.payload.constructor).to.equal(Object)
  ...
  // Changes to deeper nested objects
  describe('changes to deeper nested objects', () => {
    ...
    it('should convert nested Realm.Object objects into plain objects', () => {
    ...
    it('should convert Realm.List objects into plain arrays', () => {
    ...
    it('should convert Realm.Objects inside Realm.List objects into plain objects and arrays', () => {
      let catOwners = testRealm().objects('CatOwner')
      let initialAction = { type: 'TEST', payload: { catOwners } }
      let result = testMiddleware(initialAction)
      let catOwner = result.payload.catOwners[0]
